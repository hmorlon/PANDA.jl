...
function extract_relative_rates_full(tree::Tree, edge_trees::Array{EdgeTreeRates2,1}, rates::Array{Float64,1}; id = 1)
...
function extract_relative_rates_partial(tree::Tree, edge_trees::Array{EdgeTreeRates2,1}, rates::Array{Float64,1}; id = 1)
...
function extract_relative_rates_full(tree::Tree, edge_trees::Array{EdgeTreeRates2,1}; id = 1)
    function aux(subtree, parent_rate, x)
        if subtree.n_nodes > 0
            if length(subtree.offsprings) == 0
                pushfirst!(x,log(subtree.attributes[id]) - parent_rate)
...
function extract_relative_rates_partial(tree::Tree, edge_trees::Array{EdgeTreeRates2,1}; id = 1)
    function aux(subtree, parent_rate, x)
        if subtree.n_nodes > 0
            if length(subtree.offsprings) == 0
                #pushfirst!(x,log(subtree.attributes[id]) - parent_rate)
